...
for(k in 1:length(d.seq)){
   ...
   result <- foreach(u = 1:iterations,
                                   ...
                                   'class','nnet')) %dopar% 
      {
         ...
         mdl.bayes.X <- apply(test.sample, 1, function(val){
            return(sum(log(dpareto(val, location = 1, shape = 1))))
         })
         
         mdl.bayes.Y <- apply(test.sample, 1, function(val){
            return(sum(log(dpareto(val, location = 1.25, shape = 1))))
         })
   ...
   res.list[[k]] <- rbind(result.all, rep(NA, 21), 
...
res.list.par11.vs.par1251 <- list("d=5" = res.list[[1]],
                           "d=10" = res.list[[2]],
                           "d=25" = res.list[[3]],
                           "d=50" = res.list[[4]],
                           "d=100" = res.list[[5]],
...
writexl::write_xlsx(x = res.list.par11.vs.par1251,
                    path = "C:\\Users\\JYOTISHKA\\Desktop\\all-par11-vs-par1251.xlsx")
