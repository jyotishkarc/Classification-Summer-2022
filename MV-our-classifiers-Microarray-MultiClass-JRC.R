...
path <- "E:/JRC-2022/Classification-Summer-2022-JRC/Datasets/Microarray Database/"

MA.stats <- read_excel(paste0(path,"Microarray-DataSummary.xlsx"))
MA.implement <- MA.stats %>% arrange(J)
...
for(h in 14:length(files.MA)){
   ...
   # result.folder.path <- "G:\\Projects\\Prof. Subhajit Dutta (Summer, 2022)\\Results\\Microarray-ALL-Results-MV\\"
   
   result.folder.path <- "E:\\JRC-2022\\Classification-Summer-2022-JRC\\Results\\Real\\Microarray\\Microarray-ALL-Results-MV\\"
   
   write_xlsx(x = res.list,
              path = paste0(result.folder.path, 
                            str_remove(files.MA[h],"_database.xlsx"),
                            "-our-MV.xlsx"))
   
   
   T.mat.folder.path <- paste0(result.folder.path,"Microarray-ALL-T-matrix\\")
   ...
   time.MA[h] <- end.time.proc - start.time.proc
   print(time.MA[h])
   cat("Ended:")
   print(Sys.time())
   print(files.MA[h])
